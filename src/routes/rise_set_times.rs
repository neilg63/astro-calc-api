...
use super::super::lib::{
  traits::FromKey,
  rise_set_phases::*,
  transposed_transitions::calc_transposed_graha_transitions_from_source_refs_topo,
...
async fn list_sun_rise_set_times(params: Query<InputOptions>) -> impl Responder {
  ...
  let mut json_output = json!({ "valid": false});
  if extended_set_mode {
    ...
    json_output = json!({ "valid": sun_rise_sets.len() > 0, "date": date, "geo": geo, "sets": sun_rise_sets });
  } else {
    ...
    json_output = json!({ "valid": sun_transitions.len() > 0, "date": date, "geo": geo, "items": sun_transitions });
  }
  thread::sleep(micro_interval);
  Json(json_output)
}
